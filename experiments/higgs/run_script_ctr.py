...
def savetoys(iLabel,iSkip,iNToys,mc_out,da_out,mc_lab,da_lab,da_weights,mc_weights,iOption=0):
    ...
    nref=40000
    ts,tb=dutils.run_realistic_toy(nref,da_out,da_lab,mc_out,mc_lab,iSkip,data_weights=da_weights.double(),model_weights=mc_weights.double(),ntoys=iNToys,plot=False,width=2.0)
    data_dict["toy_sig"]  = ts
    data_dict["toy_bkg"]  = tb
    np.savez(iLabel+"nplm_toys_ctr_sup_space_tight.npz", **data_dict)
...
def main():
    ...
    parser.add_argument('--model',  dest='model',  type=str,default='higgs_cntr_sup_space_tight.npz')
