...
def plot_reconstruction(data, weight_data, ref, weight_ref, ref_preds, ref_preds_labels, t_obs=None, df=None, title='', xlabels=[], bins=[],
    ...
    for i in range(data.shape[1]):
        ...
        hD = plt.hist(data[:, i],weights=weight_data, bins=bins, label='DATA', color='black', lw=0, histtype='step', zorder=2)
        hR = plt.hist(ref[:, i], weights=weight_ref, color=colors[0],  bins=bins, lw=0, label='REFERENCE', zorder=1) #ec=colors[1],
        hN = []
        ...
        for j in range(len(ref_preds)):
            ...
            #            label=ref_preds_labels[j]+' RECO', color=colors[j+2], lw=2, s=30, zorder=4) #,edgecolor=colors[j+2])
            plt.plot(0.5*(bins[1:]+bins[:-1]),  hN[j][0], #edgecolor='black',                                                                                                                          
                        label=ref_preds_labels[j]+' RECO', color=colors[j+2]) #,edgecolor=colors[j+2]) 
        ...
        plt.scatter(x[y>0], np.log(y[y>0]), s=30, color='black', label='DATA/REF') # edgecolors='black',
        for j in range(len(ref_preds)):
            y = hN[j][0]/(hR[0]+eps)
