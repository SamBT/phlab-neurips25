...
def extract_tstat(files_dict,flk_sigmas,NTOYS=500,ftype='h5'):
    tstat_dict = {}
    for label, f5 in files_dict.items():
        if ftype == 'h5':
            f = h5py.File(f5, 'r')
            ...
            for flk_sigma in flk_sigmas:
                key = str(flk_sigma) 
                tstat_dict[label].append(np.array(f[key])[:NTOYS].reshape(-1,1))
            tstat_dict[label] = np.concatenate(tstat_dict[label],axis=1)
            if len(tstat_dict[label]) < NTOYS:
                print(f"Warning: file {f5} label {label} has less than {NTOYS} toys, has only {len(tstat_dict[label])} toys.")
            f.close()
        elif ftype == 'npy':
            data = np.load(f5)[:NTOYS]
            tstat_dict[label] = data
            if len(tstat_dict[label]) < NTOYS:
                print(f"Warning: file {f5} label {label} has less than {NTOYS} toys, has only {len(tstat_dict[label])} toys.")
        else:
...
def get_statistics(tstat_dict, flk_sigmas):
    pvalues = {}
    pvalues_min = {}
    pvalues_avg = {}
    pvalues_prod = {}
    ...
    for key in list(tstat_dict.keys()):
        ...
        for isigma,flk_sigma in enumerate(flk_sigmas):
            ...
            z_obs_val, z_empirical_val = get_zScores(t1, t2, df=np.mean(t1))
            ...
            z_empirical[key].append(np.array([z_empirical_val[0]]).reshape(1,1))
            z_empirical_p[key].append(np.array([z_empirical_val[1]]).reshape(1,1))
            z_empirical_m[key].append(np.array([z_empirical_val[2]]).reshape(1,1))
        ...
        z_empirical[key] = np.concatenate(z_empirical[key],axis=1)
        z_empirical_p[key] = np.concatenate(z_empirical_p[key],axis=1)
        z_empirical_m[key] = np.concatenate(z_empirical_m[key],axis=1)
        ...
        p_min_ref, p_min_data = NPLM_combined_pval(t1,t2,rule='min')
        p_avg_ref, p_avg_data = NPLM_combined_pval(t1,t2,rule='avg')
        p_prod_ref, p_prod_data = NPLM_combined_pval(t1,t2,rule='prod')
        
        pvalues_min[key] = p_min_data
        pvalues_avg[key] = p_avg_data
        pvalues_prod[key] = p_prod_data
        
        z_min[key] = z_emp(p_min_ref,p_min_data)
        z_avg[key] = z_emp(p_avg_ref,p_avg_data)
        z_prod[key] = z_emp(p_prod_ref,p_prod_data)

    output = {
        "pvalues": pvalues,
        "pvalues_min": pvalues_min,
        "pvalues_avg": pvalues_avg,
        "pvalues_prod": pvalues_prod,
        "z_obs": z_obs,
        "z_obs_p": z_obs_p,
        "z_obs_m": z_obs_m,
        "z_emp": z_empirical,
        "z_emp_p": z_empirical_p,
        "z_emp_m": z_empirical_m,
        "z_min": z_min,
        "z_avg": z_avg,
        "z_prod": z_prod
    }
...
def get_statistics_npy(tstat_dict):
    pvalues = {}
    z_obs = {}
    z_obs_p = {}
    z_obs_m = {}
    ...
    for key in list(tstat_dict.keys()):        
        ...
        pvalues[key] = chi2.sf(t2, df=np.median(t1))
        
        z_obs[key] = z_obs_val[0]
        z_obs_p[key] = z_obs_val[1]
        z_obs_m[key] = z_obs_val[2]
        
        z_empirical[key] = z_empirical_val[0]
        z_empirical_p[key] = z_empirical_val[1]
        z_empirical_m[key] = z_empirical_val[2]
