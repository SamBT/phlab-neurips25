...
class Hierarchical(nn.Module):
    ...
    def __init__(self, input_shape, centroids_list, widths_list, coeffs_list,
                 ...
                 positive_coeffs=False, model='Soft-SparKer2',
                 name=None, **kwargs):
    ...
    def parameters(self, n):
        parameters = []
        if self.train_coeffs:
            for m in range(self.n_layers):
                if m<=n:
                    parameters.append(self.get_coeffs_j(j=m))
        if self.train_widths:
            for m in range(self.n_layers):
                if m<=n:
                    parameters.append(self.get_widths_j(j=m))
        if self.train_centroids:
            for m in range(self.n_layers):
                if m<=n:
                    parameters.append(self.get_centroids_j(j=m))
        return parameters
