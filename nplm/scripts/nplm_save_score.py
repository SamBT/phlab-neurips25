...
anomaly_filepath = config_json["anomaly_filepath"]
...
reference_all = np.load(ref_filepath)
data_all = np.load(data_filepath)
anomaly_all = np.load(anomaly_filepath)
print(reference_all.shape, data_all.shape, anomaly_all.shape)
...
for flk_sigma in flk_sigmas:
    ...
    for i in range(Ntoys):
        ...
        data_x = np.concatenate((anomaly_all[idx_sig[:N_Sig_Pois]], data_all[idx_bkg[:N_Bkg_Pois]]), axis=0)
        ...
        t_list.append(t_tmp)
        preds_list.append(preds_tmp.reshape((-1,1))) # [N, 1]
        print(preds.shape)
        print(t_list[-1])
    tstat_dict[str(flk_sigma)]=np.array(t_list)
    preds_dict[str(flk_sigma)]=np.concatenate(preds_list, axis=1) # [N, 6]
...
for flk_sigma in flk_sigmas:
  ...
  f.create_dataset('preds_%s'%(str(flk_sigma)), data_preds_dict[str(flk_sigma)], compression='gzip')
