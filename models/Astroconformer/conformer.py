...
class PostNorm(nn.Module):
    ...
    outputs = (module(inputs) x module_factor + inputs x input_factor)
    """
    def __init__(self, module: nn.Module, encoder_dim: int, alpha: float = 1.0, **kwargs):
        super(PostNorm, self).__init__()
        self.module = module
        input_factor = torch.FloatTensor([alpha]) if alpha is not None else torch.tensor(1.)
...
class MultiHeadedSelfAttentionModule(nn.Module):
    ...
    """
    def __init__(self, encoder_dim: int, num_heads: int, dropout_p: float = 0.1, **kwargs):
        super(MultiHeadedSelfAttentionModule, self).__init__()
        self.positional_encoding = PositionalEncoding(encoder_dim)
        self.layer_norm = nn.LayerNorm(encoder_dim)
        self.attention = RelativeMultiHeadAttention(encoder_dim, num_heads, dropout_p)
        self.dropout = nn.Dropout(p=dropout_p)
...
class ConformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.modlist = nn.ModuleList([norm_dict[norm](block_dict[block](**kwargs), **kwargs) for block in encoder])
...
class ConformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([ConformerBlock(
            encoder=encoder, norm=norm, **kwargs) for _ in range(num_layers)])
